...
class lastfmApiUser extends lastfmApi {
		...
		if ( !empty($methodVars['user']) ) {
			...
			if ( $call = $this->apiGetCall($vars) ) {
				...
				if ( $call->events['total'] > 0 ) {
					...
					$this->handleError(90, 'Este Usuario No Tiene Eventos ');
					return FALSE;
				}
			}
			else {
	...
	public function getTopArtistsGlobal($methodVars) {
		// Check for required variables
		if ( !empty($methodVars['api_key']) ) {
			$vars = array(
			...
			if ( $call = $this->apiGetCall($vars) ) {
				if ( count($call->artists->artist) > 0 ) {
					$i = 0;
